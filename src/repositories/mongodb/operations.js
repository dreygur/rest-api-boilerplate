...
const find = ({ table, key = {} }) => new Promise((resolve, reject) => {
  ...
  // May break
  resolve(models[table.toLowerCase()][method](key, options)[noPaginate ? 'populate' : 'then'](populate?.path, populate?.select?.split(' ')))
...
const findOne = async ({ table, key = {} }) => new Promise((resolve, reject) => {
  ...
  models[table.toLowerCase()].findOne(key).populate(key.populate?.path, key.populate?.select?.split(' '))
    .then(res => resolve(res))
    .catch(e => reject(e));
});
